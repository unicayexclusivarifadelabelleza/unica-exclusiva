...
export async function POST(request: NextRequest) {
  try {
    ...
    // Reserve tickets (NOT sold yet - only after payment confirmation)
    await Promise.all(
      ticketNumbers.map(async (ticketNumber) => {
        ...
        return await db.ticket.update({
          where: { id: ticket.id },
          data: {
            status: 'reserved', // Changed from 'sold' to 'reserved'
            userId: user.id,
            paymentId: payment.id,
            reservedAt: new Date(),
            expiresAt: expiresAt // Tickets expire in 15 minutes if not paid
          }
        })
      })
    )
    ...
    return NextResponse.json({
      ...
      message: 'Tus boletos están reservados por 15 minutos. Por favor completa el pago.',
      instructions: `Tienes 15 minutos para pagar $${total.toLocaleString()} CLP por ${ticketNumbers.length} boleto(s). Si no pagas, los boletos se liberarán automáticamente.`
    })
